...
- **Decoder is LIVE and committed** (`esphome/decoder.yaml`, vendored component `esphome/components/inputs/esp32-spa.h`). Reads Water Temp, Set Temp, Heater, Light, Blower (with caveats — see Decoder issues), and Error code. Device IP **192.0.2.10** on WiFi SSID **`YOUR_IOT_WIFI_SSID`** (IoT VLAN — NOT "YOUR_WIFI_SSID").
...
- **OTA from this box**: open the tunnel as a background task —
  `ssh -i ~/.ssh/ha_bridge -N -L 3232:192.0.2.10:3232 -L 6053:192.0.2.10:6053 root@203.0.113.10`
...
- **Read live state**:
  `ssh -i ~/.ssh/ha_bridge root@203.0.113.10 'curl -s --max-time 8 http://192.0.2.10/events'`.
